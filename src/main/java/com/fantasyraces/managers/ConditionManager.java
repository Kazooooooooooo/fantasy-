...
public class ConditionManager {
    ...
    private static final int EFFECT_DURATION = 60; // 3 seconds of effect duration (refreshed every 2s)

    // All biomes considered "cold" for the IN_COLD_BIOME trigger
        ...
        for (Player player : Bukkit.getOnlinePlayers()) {
            ...
            for (Condition condition : race.getConditions()) {
                if (evaluateTrigger(player, condition)) {
                    // Apply potion effect if present
                    if (condition.getEffect() != null) {
                        applyPotionEffect(player, condition.getEffect());
                    }
                    // Apply special effect if present
                    if (condition.hasSpecialEffect()) {
                        applySpecialEffect(player, condition.getSpecialEffect());
                    }
                }
            }
        }
    ...
    private void applySpecialEffect(Player player, Condition.SpecialEffect effect) {
        switch (effect) {
            case SET_ON_FIRE -> {
                // Only set on fire if not already burning and not fire resistant
                if (player.getFireTicks() <= 0
                        && player.getPotionEffect(PotionEffectType.FIRE_RESISTANCE) == null) {
                    player.setFireTicks(TICK_INTERVAL + 20); // keep burning until next tick
                }
            }
            case FREEZE -> {
                // Increase freeze ticks — max is 140 ticks for full freeze visual
                int current = player.getFreezeTicks();
                int max = player.getMaxFreezeTicks();
                if (current < max) {
                    player.setFreezeTicks(Math.min(current + 30, max));
                }
            }
    ...
    /**
     * Removes all passive and condition effects associated with a race from a player.
     */
