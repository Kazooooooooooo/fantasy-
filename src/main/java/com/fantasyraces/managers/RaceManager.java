...
public class RaceManager {
    ...
    private Condition parseCondition(Map<?, ?> map) {
        ...
        String effectName       = (String) map.get("effect");         // potion effect (optional)
        String specialEffectName = (String) map.get("special-effect"); // physical effect (optional)

        if (triggerName == null) return null;
        if (effectName == null && specialEffectName == null) {
            plugin.getLogger().warning("Condition with trigger '" + triggerName + "' has no effect or special-effect — skipping.");
            return null;
        }
        ...
        try {
            ...
            return new Condition(trigger, yValue, biomes, world, effect, specialEffect);
        } catch (IllegalArgumentException e) {
            plugin.getLogger().warning("Invalid condition trigger: " + triggerName);
            return null;
        }
    }
